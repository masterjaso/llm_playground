...
def validate(*, run_dir: Path, profiles: list[str], fresh_process: bool) -> dict[str, Any]:
    ...
    payload = {
        "schema_version": 1,
        "receipt_type": "dense2moe-fresh-process-reload-parity",
        "status": "BLOCKED",
        "blocker_code": "FRESH_PROCESS_LOGIT_PROBE_REQUIRED",
        "fresh_process": True,
        "profiles": profiles,
        "assembly_receipts": receipts,
        "logit_parity_measured": False,
        "message": "assembled inputs exist, but a fresh-process source-backed logits probe is required before promotion",
    }
...
def main() -> int:
    ...
    return 0 if result["status"] == "RELOAD_PARITY_GREEN" else 2
