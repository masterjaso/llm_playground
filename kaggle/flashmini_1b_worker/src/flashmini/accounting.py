...
class ParameterCounts:
    ...
    # Explicit production-report categories.  They default to zero so older
    # callers that construct the dataclass positionally remain compatible.
    dense_shared: int = 0
    attention: int = 0
    gdn: int = 0
    moe: int = 0
    active_moe_expert_per_token: int = 0
    kvc: int = 0
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "gdn": self.gdn,
            "moe": self.moe,
            "active_moe_expert_per_token": self.active_moe_expert_per_token,
            "kvc": self.kvc,
        }
...
def count_parameters(model: nn.Module, config: FlashMiniConfig) -> ParameterCounts:
    ...
    categories = {"dense_shared": 0, "attention": 0, "gdn": 0, "moe": 0}
    modules = dict(model.named_modules())
    for name, parameter in model.named_parameters():
        if name.startswith(("embed", "head", "ple")):
            continue
        if ".moe." in name:
            categories["moe"] += parameter.numel()
        elif ".mixer." in name:
            owner = modules.get(name.split(".mixer.", 1)[0] + ".mixer")
            categories["attention" if type(owner).__name__ == "CausalAttention" else "gdn"] += parameter.numel()
        else:
            categories["dense_shared"] += parameter.numel()
    active_moe = 0
    for name, parameter in model.named_parameters():
    ...
    return ParameterCounts(
        ...
        dense_shared=categories["dense_shared"], attention=categories["attention"],
        gdn=categories["gdn"], moe=categories["moe"],
        active_moe_expert_per_token=active_moe,
        kvc=0,
    )
